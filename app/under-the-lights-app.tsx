...
function TeamMark({ initials: mark, name, position, competition, home = false }: { initials: string; name: string; position: number | null; competition: string; home?: boolean }) {
  const { t } = useI18n();
  return <div className="team-mark"><div className={home ? "team-badge home" : "team-badge"}>{mark}</div><strong>{name}</strong><span>{t("{position} in {competition}", { position: position ? `#${position}` : t("Unranked"), competition })}</span></div>;
}
  ...
  return (
    <section className="inner-page">
      ...
      <section className="profile-connection" aria-labelledby="soccerverse-account-title">
        <div><SoccerBall size={28} weight="duotone" /><div><h2 id="soccerverse-account-title">{t("Your Soccerverse account")}</h2><p>{t("Add your in-game username so players can open your public Soccerverse profile from the leaderboard.")}</p></div></div>
